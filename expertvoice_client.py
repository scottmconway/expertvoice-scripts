...
def flatten_taxonomy(category: Dict, parent_name: Optional[str] = None) -> List[Dict]:
    result = list()
    if parent_name:
    ...
    if "taxonomy" in category:
        ...
        for sub_category in category["taxonomy"]:
            result.extend(flatten_taxonomy(sub_category, parent_name=category["name"]))
...
class ExpertvoiceClient:
    ...
    def get_categories(self, depth: Optional[int] = None) -> List[Dict]:
        ...
        for category_group in res["browse"]:
            categories.extend(flatten_taxonomy(category_group))
